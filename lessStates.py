...
DT = 0.2
DRONE_RADIUS = 0.3
NUM_DRONES = 4  # Change this to 3, 4, 5, etc. to test swarm scaling!
...
# ---------------- PRUNED DISCRETIZATION STATE LOGIC (576 States) ----------------
...
class Environment:
    ...
    def reset(self, randomize_positions=True):
        ...
        for i, drone in enumerate(self.drones):
            ...
            drone.goal = -drone.position.copy()  # Goal is exactly opposite
    ...
    def step(self, actions):
        ...
        # Apply actions and move
        for i, drone in enumerate(self.drones):
            if goals_reached[i]:
                drone.velocity = np.array([0.0, 0.0])
            else:
                drone.velocity = ACTIONS[actions[i]]
        ...
        # Calculate rewards and nearest-neighbor observations
        # Calculate rewards and nearest-neighbor observations
        for i, drone in enumerate(self.drones):
            closest = get_closest_drone(drone, self.drones)
            ...
            # 1. BOOSTED PROGRESS REWARD: ~+10 points per step for perfect heading
            progress = old_dists[i] - new_dists[i]
            reward = progress * 50.0  
            
            # 2. BASE TIME PENALTY: Stop dawdling
            reward -= 1.0

            # 3. REDUCED WARNING PENALTY: Scary, but not worse than running away
            if dist_between < WARNING_DISTANCE:
                reward -= 5.0 * (1.0 - drone.priority)

            # 4. MASSIVE COLLISION PENALTY: Fatal mistakes hurt
            if collision:
                reward -= 500.0 * (1.0 - drone.priority + 0.1)

            # 5. MASSIVE GOAL REWARD
        ...
        # Episode ends if everyone reaches goal OR any collision happens
...
def test_and_visualize(env, agent):
    ...
    ax.set_title(f"Learned Trajectories ({env.num_drones} Drones)")
